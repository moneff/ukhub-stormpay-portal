...
- OIDC Well Known endpoint: https://auth1.sb.stormpay.dev/.well-known/openid-configuration
- baseUrl: https://auth1.sb.stormpay.dev/

### Step by Step guide to connect to sandbox

#### Step 1: Pre-Requisites for TPP
Ensure that the following pre-requisites are met before onboarding onto StormPay.
...
response = requests.post("https://ob-api1.sb.stormpay.dev/dynamic-client-registration/v3.1/register",
...
Once a TPP successfully registers to StormPay, they can download the environment file for their client via an StormPay helper api

```
Endpoint: https://ob-api1.sb.stormpay.dev/ozone/v1.0/postman/environment
...
https://rs1.sb.stormpay.dev
https://auth1.sb.stormpay.dev/token
...
Once the PSU consent is successful, StormPay will redirect back to the redirect URI. Copy the Authcode from the URL
