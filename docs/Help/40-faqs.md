...
Open Banking is a reform, called for by the Competitions & Market Authority (CMA), which mandates StormPay and the eight other largest current account providers (CMA9) to securely share customer account data and initiate payments with registered third party providers (TPPs) provided the customer has given their consent.
...
(1) Make sure you have registered your SSA in StormPay Developer Portal and the subscription of the Accounts Service Provider API and/or Payments Service Provider API is approved by StormPay
