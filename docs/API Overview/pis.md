...
## Base URL
The base URL for all AIS APIs is: `https://rs1.ob.stormpay.io/open-banking/v3.1/pisp/**`

## Supported Payment Types
The StormPay API currently only supports:
- Domestic Payments
- Domestic Scheduled Payments
- Domestic Standing Orders

The StormPay API __does not__ support:
...
- There is no MAX `InstructedAmount/Amount` mandated by StormPay API.
- 50,000 GBP is the default maximum when opening a StormPay account, but thresholds can be managed by the customer. StormPay suggest PISP notify the PSU that the same limits apply as in their StormPay app. It is possible from time to time that `domestic-payment-consents` is authorised, but the payment initiation fails due to account limits.
...
- Notify PSU that only payments to existing recipients are permitted and in the event this condition is not met StormPay will not authorise the consent and display error modal to user
- Integrate with StormPay beneficiaries API and can confirm that the recipient already exists before sending the consent to allow for a better user experience
...
However, this field is not mandatory so we suggest PISP simply not include this field and StormPay will stage consent as a faster payment.
...
- `Initiation/Frequency` supported by StormPay  are:
  - `IntrvlMnthDay:01:xx`
  - `IntrvlWkDay:01:xx`
...
- last two digits after IntrvlMnthDay are not considered or validated by StormPay  (payment will be made as per the day in FirstPaymentDateTime)
...
- last two digits after IntrvlMnthWkDay are not taken in to account by StormPay (payment will be made as per the day in FirstPaymentDateTime)